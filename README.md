...
On ancient systems, like Fedora 1 and Ubuntu 4, you will need to build Bash immediately. Run `setup-bash.sh` for a reduced functionality Bash. Nearly all other systems have a new enough version of Bash.
...
Some older versions of system header files cause a broken compile due to `__THROW` on C functions. The system headers are actually OK, the problem is Gnulib. Gnulib sets `__THROW` to the unsupported `__attribute__ ((__nothrow__))` and it breaks the compile. Affected versions include the header supplied with Fedora 1 and Ubuntu 4.

If you encounter a build error *"error: parse error before '{' token"*, then open the header and add the following after the last include. The last include should be `<features.h>`, give or take.
...
The system headers that are affected include:
...
OpenSSL 1.1.x has a lot of problems on older systems due to a dependency on Perl 5.24 and above. On modern systems with adequate Perl the OpenSSL recipe will build and install OpenSSL 1.1.x. On older systems with an old version of Perl, the OpenSSL recipe will build and install OpenSSL 1.0.2.

OpenSSL 1.0.2u was last maintained on December 2019. It is now end-of-life. OpenSSL 1.0.2 from December 2019 is a lot better than the OpenSSL version on older systems, like OpenSSL 0.9.8 through 1.0.1.

### Nettle

Nettle is a constant source of problems. It is not well tested. It has chronic configuration problems and the self tests usually fail to run. Things only get worse on non-Linux systems.

Note to future maintainers: test your shit.
