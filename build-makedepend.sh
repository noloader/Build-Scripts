...
# This script builds makedepend from sources.

MAKED_VER=1.0.6
MAKED_TAR=makedepend-${MAKED_VER}.tar.gz
MAKED_DIR=makedepend-${MAKED_VER}
PKG_NAME=makedepend
...
rm -rf "$MAKED_DIR" &>/dev/null
gzip -d < "$MAKED_TAR" | tar xf -
cd "$MAKED_DIR" || exit 1
